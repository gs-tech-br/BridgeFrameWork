...
type
  TConnectionFirebird = class(TBaseConnection)
  strict private
...
uses
  ...
  Bridge.Connection.Generator.Firebird;

{ TConnectionFirebird }

constructor TConnectionFirebird.Create(ACredentials: IConnectionCredentialsProvider;
  ADriverConfig: IDriverConfigProvider;
  ASQLGenerator: ISQLGenerator);
begin
  if not Assigned(ASQLGenerator) then
    ASQLGenerator := TFirebirdGenerator.Create;

  inherited Create(ACredentials, ADriverConfig, ASQLGenerator);
  
  if not Assigned(FDriverConfig) then
    FDriverConfig := TDefaultDriverConfig.Create(dtFirebird);

  ConfigureDriver;
  FConnection := Self.CreateConnection;
end;

destructor TConnectionFirebird.Destroy;
begin
  FDriverLink.Free;
  inherited;
end;

procedure TConnectionFirebird.ConfigureDriver;
begin
...
function TConnectionFirebird.CreateConnection: TFDConnection;
begin
...
function TConnectionFirebird.getColumns(const ATable: string): TStringList;
const
  LSQL = 
    'SELECT ' +
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        LCachedList.Add(Trim(LField.AsString));
        LQuery.Next;
      end;
...
function TConnectionFirebird.getId(const ATable: string): Integer;
var
  LQuery: TFDQuery;
begin
...
function TConnectionFirebird.getSeq(const ATable, AColumnName: string): Variant;
var
  LValue: Variant;
begin
...
initialization
  TConnectionFactory.RegisterConnection(dbFirebird, TConnectionFirebird);
